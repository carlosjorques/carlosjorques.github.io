...
export const GET: APIRoute = async ({ site }) => {
	const base = site ?? new URL('https://carlosjorques.com');
	const staticRoutes = ['', 'about/', 'work/', 'writing/', 'contact/', 'cv/', 'privacy/'];
	const writing = (await getCollection('writing')).filter((entry) => !entry.data.draft).map((entry) => `writing/${entry.id.replace(/\.md$/, '')}/`);
	const work = (await getCollection('projects')).map((entry) => `work/${entry.id.replace(/\.md$/, '')}/`);
