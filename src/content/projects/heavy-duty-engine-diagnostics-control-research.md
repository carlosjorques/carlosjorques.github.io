...
deliverables: Delivered experimentally validated methods for in-cycle pilot-mass estimation, pilot-misfire detection, predictive closed-loop combustion control, stochastic set-point optimization, model adaptation, and FPGA resource evaluation on a physical heavy-duty engine. The work produced a doctoral thesis, conference contributions, and a research platform that connected advanced algorithms to deterministic embedded execution.
metrics:
  - label: Experimental set-up
    value: 6-cylinder, 12.7 L Scania D13 test bench
  - label: Efficiency increase
    value: +1.8% indicated thermal efficiency in an experimental set-point study
  - label: Diagnostic results
    value: Up to 96% misfire classification in experimental conditions
...
references:
  ...
  - label: Design and Optimization of In-Cycle Closed-Loop Combustion Control with Multiple Injection
    href: https://lup.lub.lu.se/search/publication/afeb6cfc-2aa4-4230-b51e-492b9376791e
    context: Lund University PhD thesis, 2021. Research thesis covering the modeling, diagnostics, optimization, and control methods.
reflection: "The research lesson was that a model is useful only when its uncertainty is visible, a diagnostic is useful only when it can support a decision, and an advanced controller is useful only when it can execute deterministically on real hardware. That is the broader value of virtual sensing and closed-loop optimization: advanced methods become useful to physical systems when they can handle variation, shorten the learning loop, and remain testable beyond simulation."
---
