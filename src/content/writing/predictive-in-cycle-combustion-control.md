...
My [PhD thesis](/research/), *Design and Optimization of In-Cycle Closed-Loop Combustion Control with Multiple Injections*, investigated this control problem for compression-ignition engines with pilot-main injection. A central part of the work was the development of predictive closed-loop controllers that could regulate combustion on a cycle-resolved time scale, using in-cylinder pressure measurements, model-based estimation, online adaptation, and FPGA implementation.

The goal was not only to control combustion more accurately. The goal was to make control decisions early enough to affect the combustion event that was still unfolding.
...
Combustion control has an intrinsic timing problem. The controller action, such as a fuel injection command, occurs before the measured combustion output is fully available. In a conventional feedback loop, this creates a delay: the controller observes the error after the cycle and compensates in the next one.

For in-cycle control, that delay must be overcome. The controller cannot wait until the full pressure trace or complete heat-release event is available. It must predict how the rest of the cycle will evolve from the partial information already observed.

This is why predictive control is central to in-cycle combustion regulation. The controller needs a model that can estimate the relationship between the current combustion state, the remaining injection command, and the final combustion output.

In the thesis, predictive models were used to overcome the delay between actuator input and measured output. The controller used early pressure-derived information to estimate the state of combustion, predict the effect of candidate fuel-injection actions, and select an injection correction before the available control window closed.
...
This turned the controller into a sequence of tightly connected operations: measure pressure, estimate early combustion states, predict the remaining response, compute the required correction, and apply the correction before the cycle is over.
...
The predictive control strategy used this model information to calculate the injection correction needed to reduce cycle-to-cycle variation. Rather than waiting for the final error, the controller anticipated the effect of pilot-combustion variation and acted before the main combustion event was complete.

This is the essence of predictive in-cycle control: use the information available now to estimate what will happen next, then act while there is still control authority.
...
The research also showed that adaptation must be designed carefully. If too many model parameters are adapted independently, the system can become difficult to calibrate and may introduce undesirable transients. A reduced multi-cylinder adaptation method was therefore proposed to reduce complexity: some parameters were adapted per cylinder, while others were shared across cylinders.
...
The thesis reported that predictive in-cycle control reduced pilot start-of-combustion variation from about +/-1 crank-angle degree in open-loop operation to about +/-0.4 crank-angle degrees. Burnt pilot mass variation was reduced from about +/-1.5 mg to about +/-0.6 mg. Main start-of-combustion variation was reduced from about +/-0.4 to +/-0.3 crank-angle degrees, and IMEP variation was reduced from about +/-0.8 bar to +/-0.2 bar.
...
<section class="metric-grid" aria-label="Selected quantitative results from predictive in-cycle combustion control">
	<div class="metric-card">
		<strong>+/-1 to +/-0.4 CAD</strong>
		<span>Pilot SOC variation</span>
	</div>
	<div class="metric-card">
		<strong>+/-1.5 to +/-0.6 mg</strong>
		<span>Burnt pilot mass variation</span>
	</div>
	<div class="metric-card">
		<strong>+/-0.4 to +/-0.3 CAD</strong>
		<span>Main SOC variation</span>
	</div>
	<div class="metric-card">
		<strong>+/-0.8 to +/-0.2 bar</strong>
		<span>IMEP variation</span>
	</div>
	<div class="metric-card">
		<strong>+1.5 +/- 0.6 to 0 +/- 0.4 CAD</strong>
		<span>Pilot misfire SOC compensation</span>
	</div>
	<div class="metric-card">
		<strong>-0.5 +/- 0.4 to 0 +/- 0.2 bar</strong>
		<span>Load compensation</span>
	</div>
	<div class="metric-card">
		<strong>+0.42 pp</strong>
		<span>Direct efficiency gain</span>
	</div>
	<div class="metric-card">
		<strong>Up to +1.8 pp</strong>
		<span>Indirect low-load gain</span>
	</div>
</section>
...
The control algorithms were implemented on FPGA hardware. This was not just an implementation detail; it shaped the control design.

In-cycle combustion control requires computation at crank-angle-resolved time scales. The system must acquire pressure data, process signals, estimate combustion states, update predictions, select controller actions, and synchronize injection commands with engine position. These operations must occur with deterministic timing.

The thesis used FPGA implementation because it allowed fast, parallel, and deterministic computation. At the same time, FPGA hardware introduced resource constraints. The models and estimators had to be simplified enough to fit within available hardware resources while still being accurate enough for control.

This required careful design choices: simplified combustion models, reusable computation blocks, fixed-point representation, modular controller structure, and explicit quantification of hardware requirements.
...
The thesis proposed a modular controller design coordinated by a finite-state machine. This structure synchronized measurements, model-based predictions, active controller selection, actuator output, and strategy coordination.

The modular architecture made the system easier to calibrate and extend. Individual controller modules could be developed, tested, and integrated sequentially. The supervisory logic could determine which control mode was active depending on observability, controllability, diagnostic state, and operating condition.

This is particularly important for in-cycle control because the controller may need to switch behavior quickly. For example, normal predictive control may be used when pilot combustion is observable and the main injection remains adjustable. Misfire-compensation logic may be activated when a pilot misfire is detected. A fallback mode may be required when the cycle is no longer controllable.

The finite-state-machine structure allowed these modes to be coordinated in real time.
...
The thesis investigated both direct and indirect efficiency optimization methods. A direct in-cycle optimization method increased indicated efficiency by +0.42 percentage points experimentally. An indirect stochastic optimization method, using the reduced dispersion enabled by in-cycle control, increased indicated efficiency by +0.6 percentage points at medium loads and up to +1.8 percentage points at low loads.
